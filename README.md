# Hi, I'm Amna Saeed 👋  
🎓 BSCS Student | Learning C++, Python, and Image Processing  
💻 Interested in Graphic Designing, Software Development AI, Generative AI, Agentic AI, LLM.   
📂 Check out my projects below!  

## 🚀 My Projects  
- *Library Management System (C++)*  
- *Image Processing Techniques (Python)* 
- *Data Mining Algorithms (Python)*   
- *Operating System Scheduling (C++)* 
- *Car Pool Sytem (Website) (Python for bckend)*
- *Accident Detection System using CCTV- Web Based Project (Python for backend)*
- *Employee Focus Detection (Python)*

🔗 *GitHub Repositories*: [Click Here](https://github.com/amnasaeed1231?tab=repositories)
